...
class MenuController extends BaseController {
    ...
    public function add() {
        if (IS_POST) {
            ...
            $originList = D('Menu')->order('sort asc')->select();
            $fid = '';
            $id = I('get.id');
            if (!empty($id)) {
                $fid = $id;
    ...
    public function open() {
        $id = I('post.id');
        $res = D('Menu')->where(array('id' => $id))->save(array('hide' => 0));
        if ($res === false) {
            $this->ajaxError('操作失败');
        } else {
            $this->ajaxSuccess('添加成功');
    ...
    public function close() {
        $id = I('post.id');
        $res = D('Menu')->where(array('id' => $id))->save(array('hide' => 1));
        if ($res === false) {
            $this->ajaxError('操作失败');
        } else {
            $this->ajaxSuccess('添加成功');
    ...
    public function edit() {
        if (IS_GET) {
            ...
            $res = D('Menu')->where(array('id' => $postData['id']))->save($postData);
            if ($res === false) {
                $this->ajaxError('操作失败');
            } else {
    ...
    public function del() {
        $id = I('post.id');
        $childNum = D('Menu')->where(array('fid' => $id))->count();
        if ($childNum) {
            $this->ajaxError("当前菜单存在子菜单,不可以被删除!");
        } else {
            D('Menu')->where(array('id' => $id))->delete();
            $this->ajaxSuccess('编辑成功');
        }
    }
