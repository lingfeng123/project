...
class BaseController extends Controller
{
    ...
    public function _initialize()
    {
        ...
        //获取记录器
        $this->log = \Logger::getLogger('V1_1');
    }
    ...
    public function verifyToken()
    {
        $token = I('post.token', '');
        $uid = I('post.uid', '');
        $method_uri = CONTROLLER_NAME . '/' . ACTION_NAME;

        //不验证token的方法
        $not_verify_uri = C('NOT_VERIFY');
        if (!in_array($method_uri, $not_verify_uri)) {
            //验证登录用户合法性
            $vlidate_rs = AuthSign::verifyMemToken($token, $uid);
            if (!$vlidate_rs) {
                Response::error(ReturnCode::TOKEN_EXPIRED, '登录状态已失效');
            }
        }
    }
